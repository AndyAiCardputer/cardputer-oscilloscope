 ...
 * Display: 240x135 TFT via sprite buffer (no flicker).
 *
 * Controls:
 *   R       - Run / Stop
...
// --- Pin Configuration ---
#define ADC_PIN      1   // GPIO 1 = Grove G1 on Cardputer (ADC1_CH0)
#define TEST_SIG_PIN 2   // GPIO 2 = Grove G2 -- built-in test signal output

// --- Display Layout ---
#define SCREEN_W     240
#define SCREEN_H     135
#define GRAPH_X      20   // left margin for voltage labels
#define GRAPH_Y      2    // top margin
#define GRAPH_W      200  // waveform area width in pixels
#define GRAPH_H      100  // waveform area height in pixels
#define STATUS_Y     (GRAPH_Y + GRAPH_H + 4)
...
int vScaleIdx = 3;             // default 1000 mV/div
...
void setup() {
    ...
    // Release I2C from Grove pins (safe on v1.1 -- keyboard is GPIO matrix)
    Wire.end();
    ...
    // Initialize DMA ADC BEFORE any Arduino ADC calls
    setupFastADC();

    // Test signal generator on G2
    gpio_reset_pin(GPIO_NUM_2);
    ...
    Serial.printf("Screen: %dx%d | Graph: %dx%d\n", SCREEN_W, SCREEN_H, GRAPH_W, GRAPH_H);
    Serial.println("Connect G1 to G2 with a wire to see test signal!");
}
...
void drawTriggerLine() {
    ...
    if (y >= GRAPH_Y && y <= GRAPH_Y + GRAPH_H) {
        ...
        canvas.fillTriangle(GRAPH_X - 5, y, GRAPH_X - 1, y - 3, GRAPH_X - 1, y + 3, COL_TRIGGER);
    }
}

// =============================================
// Draw Measurements (right side)
// =============================================
void drawMeasurements() {
    int x = GRAPH_X + GRAPH_W + 3;
    int y = GRAPH_Y;
    ...
    y += 12;
    canvas.setCursor(x, y);
    canvas.printf("H%.1f", measuredVmax);

    // Vmin
    y += 12;
    canvas.setCursor(x, y);
    canvas.printf("L%.1f", measuredVmin);

    // Frequency
    if (showFreq && measuredFreqHz > 0) {
        y += 16;
        canvas.setTextColor(COL_HIGHLIGHT);
        canvas.setCursor(x, y);
        if (measuredFreqHz >= 1000) {
            canvas.printf("%.1fk", measuredFreqHz / 1000.0f);
        } else {
            canvas.printf("%.0fHz", measuredFreqHz);
        }
    }
    ...
    // Voltage labels on left side (Y axis)
    float mvPerDiv = vScaleSteps[vScaleIdx];
    float totalMv = mvPerDiv * 4;
    canvas.setTextColor(COL_TEXT_DIM);

    for (int i = 0; i <= 4; i++) {
        float v = (totalMv - i * mvPerDiv) / 1000.0f;
        int ly = GRAPH_Y + i * (GRAPH_H / 4) - 3;
        canvas.setCursor(0, ly);
        if (v >= 1.0f) {
            canvas.printf("%.1f", v);
        } else {
            canvas.printf(".%d", (int)(v * 10));
        }
    }
}
...
void drawStatusBar() {
    ...
    if (usPerDiv >= 1000) {
        canvas.printf("T:%dms", usPerDiv / 1000);
    } else {
        canvas.printf("T:%dus", usPerDiv);
    }
    ...
    if (mvPerDiv >= 1000) {
        canvas.printf("V:%.1fV", mvPerDiv / 1000.0f);
    } else {
        canvas.printf("V:%dmV", mvPerDiv);
    }
    ...
    if (testSignalOn) {
        ...
        if (tf >= 1000) canvas.printf("%dk", tf / 1000);
        else canvas.printf("%d", tf);
    }
    ...
    canvas.setTextColor(COL_TEXT_DIM);
    canvas.setCursor(0, y);
    canvas.print("R ;.T +-V A:auto C:cal G:sig M:mode");
}
...
void handleKeyboard() {
    ...
    if (M5Cardputer.Keyboard.isKeyPressed('c')) runCalibration();
...
void runCalibration() {
    ...
    if (vpp < 500) {
        ...
        Serial.println("[CAL] FAILED - no signal. Connect G1 to signal source!");
        return;
    }
